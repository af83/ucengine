...
test_presence_create_missing_auth() ->
	...
	tests_utils:put("/presence/testorg/participant.user@af83.com", Params).
...
test_presence_create_not_found_org() ->
	...
	tests_utils:put("/presence/unexistentorg/participant.user@af83.com", Params).
...
test_presence_create_not_found_user() ->
	...
	tests_utils:put("/presence/testorg/unexistent.user@af83.com", Params).
...
test_presence_create_unauthorized([_, {UglyUid, _}]) ->
    Params = [{"metadata[nickname]", "PasswordParticipant"},
	      ...
	      {"credential", "pwd"}],
    {struct,[{"error", "unauthorized"}]} =
	tests_utils:put("/presence/testorg/" ++ UglyUid, Params).
...
test_presence_close() ->
    ...
    {struct,[{"result", Sid}]} = tests_utils:put("/presence/testorg/" ++ Uid, Params),

    ParamsDelete = [{"uid", Uid},
		    {"sid", Sid}],
    {struct, [{"result", "ok"}]} =
	tests_utils:delete("/presence/testorg/" ++ Uid ++ "/" ++ Sid, ParamsDelete).

test_presence_close_unauthorized([_, {UglyUid, UglySid}]) ->
    ParamsDelete = [{"uid", UglyUid},
		    {"sid", UglySid}],
    {struct, [{"error", "unauthorized"}]} =
	tests_utils:delete("/presence/testorg/" ++ UglyUid ++ "/" ++ UglySid, ParamsDelete).
...
test_presence_close_not_foundsid() ->
    ...
    ParamsDelete = [{"uid", Uid},
		    {"sid", Sid}],
    {struct, [{"error", "not_found"}]} =
	tests_utils:delete("/presence/testorg/" ++ Uid ++ "/unexistentsid", ParamsDelete).
