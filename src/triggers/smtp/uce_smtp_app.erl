...
config(login) ->
    case application:get_env(uce_smtp, login) of
        {ok, Term} -> Term;
        undefined -> no_login
    end;
config(Item) ->
    case application:get_env(uce_smtp, Item) of
        {ok, Term} -> Term;
        undefined ->
            error_logger:error_msg("uce_smtp not correctly configured: missing ~p",
                                   [Item]),
            exit(uce_smtp_misconfigured)
    end.

need_ssl() ->
    {_Host, Port} =  uce_config:get_local_option(smtp_host),
    need_ssl(Port).
